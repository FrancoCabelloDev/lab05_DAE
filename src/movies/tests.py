...
def get_collaborative_recommendations(user):
    # Paso 1: Obtener las películas que ha calificado el usuario
    user_ratings = Rating.objects.filter(user=user)
    user_movies = set(r.movie for r in user_ratings)

    # Paso 2: Obtener usuarios que han calificado las mismas películas
    similar_users = Rating.objects.filter(movie__in=user_movies).exclude(user=user).values_list('user', flat=True).distinct()

    # Paso 3: Buscar películas que estos usuarios han calificado, pero que user aún no ha visto
    candidate_ratings = Rating.objects.filter(user__in=similar_users).exclude(movie__in=user_movies)

    # Paso 4: Ordenar por valor de rating (de mayor a menor)
    sorted_ratings = candidate_ratings.order_by('-value')

    # Paso 5: Obtener las películas únicas en ese orden
    recommended_movies = []
    seen = set()
    for rating in sorted_ratings:
        if rating.movie.id not in seen:
            recommended_movies.append(rating.movie)
            seen.add(rating.movie.id)
